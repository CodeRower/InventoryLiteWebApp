(function (app) {
    ...
    app.controller('crateReportCtrl', crateReportCtrl);

    crateReportCtrl.$inject = ['$scope', '$modal', 'apiService', 'notificationService'];

    function crateReportCtrl($scope, $modal, apiService, notificationService) {
        ...
        function search(page) {
            ...
            apiService.post('/api/crateEntry/', filterData,
        ...
        function purchasesLoadCompleted(result) {
            ...
            if ($scope.filterPurchases && $scope.filterPurchases.length) {
                notificationService.displayInfo(result.data.Items.length + ' crate entrys found');
            }
